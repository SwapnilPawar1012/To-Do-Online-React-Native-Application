...
type UpdateTaskProps = {
  navigation: NativeStackNavigationProp<DrawerParamList, 'UpdateTask'>;
};

const UpdateTask = ({route, navigation}: any) => {
  const {item} = route.params; // This will give you the `item` you passed

  const {HandleUpdateTask} = TaskController();
  ...
  const HanldeOnSubmit = async () => {
    ...
    const resp = await HandleUpdateTask(data);
    if (resp.success) {
      Snackbar.show({
        text: resp.message || 'Task updated successfully!',
